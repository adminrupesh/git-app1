...
- **Telegram "Unauthorized user"**: The gateway log prints the numeric user ID. See `references/telegram-gateway.md` for the full auth setup and pitfalls.
...
| Gateway Telegram auth | `references/telegram-gateway.md` in this skill |
...
### Terminal Limitations (No TTY)

Hermes's `terminal` tool runs without a TTY, which means **interactive prompts (sudo password entry, Homebrew installation wizard, any TUI) will fail or hang**. Plan workarounds ahead of time:

- **Avoid interactive installers** that require typing into prompts (e.g. `brew install` when Homebrew is not yet installed and requests sudo)
- **Prefer `--yes`/`-y` flags** and pre-configure any CLI that normally asks questions
- **Homebrew on macOS**: If Homebrew is missing, `curl | bash` install scripts will require sudo and hang. Instead use `open` to launch the official installer PKG, or guide the user to install it manually.
- **macOS App Store installs**: The `mas` CLI exists but must be pre-installed; without it, use `open macappstore://<app-id>` to bring the App Store GUI to the foreground and let the user click Install.
